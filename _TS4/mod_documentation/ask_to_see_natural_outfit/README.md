# 🧍 Ask to see Natural Outfit

This mod is primarily a demonstration of how to add custom mixer interactions to affordance lists in The Sims 4.

It modifies the affordance lists in these tunings:
* `trait_teen`, ...
* `goal_CAD_IntSocial_SmallTalk`
* `social_Mixers_Toddler_Friendly_NonTouching`

Each of these lists includes the interaction:
* `mixer_social_targeted_friendly_MiddleScore_SeeOutfit_Everyday`, ...

The showcase registers yet another SeeOutfit interaction, illustrating how new social mixers can be integrated alongside existing ones.

## 🌿 Disclaimer: What Is Naturism?
Naturism is a lifestyle and philosophy that embraces **non-sexual nudity** to promote body positivity, respect for others, and harmony with nature.  
Cultural taboos and media portrayals often distort its meaning. For a deeper understanding, consult trusted sources or AI tools like Copilot, Siri, or Gemini.

This mod is designed to support naturist values in a respectful, mature, and inclusive way.

> **Please Note:**  
> This mod is not intended to depict or promote sexual content.  
> If you are uncomfortable with naturism or nudity, please do not download or use this mod.

## 🧩 Features
Adds a single interactions under **Friendly > Small Talk > Ask to see Outfit ...**:
* `Natural` - This string is localized (TS4 string table, this mod has no string tables)

These interactions are **not sexual** in nature, though perceptions may vary.

### 👶 Age Behavior
* The default version is based on 'Sleep' outfit and has some checks.
* The NSFW version is based on the 'Everyday' outfit interaction which may be NSFW as there seem to be no age checks.
To activate it remove `cfg/add_mixers.txt` anc copy `cfg.nsfw/add_mixers_nsfw.txt` into `cfg`.

## 🐞 Known Issues
* Many gamers get embarrassed seeing the vanilla censor mosaic.
* Sims may revert to their previous outfits very shortly after switching into their bathing outfit.
* Sims may get an embarrassed moodlet, especially when using the NSFW version.
